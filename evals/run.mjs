...
console.log(`\n${passed}/${total} fixtures passed all hard checks. Mean next-action quality: ${qOverall ? qOverall.toFixed(2) : 'n/a'}/5. Crisis caught: ${crisisCaught}/${crisis.length}.`)
...
- **Crisis caught: ${crisisCaught}/${crisis.length}** crisis dumps correctly routed to the support card.
...
// Non-zero exit if the safety net or quality bar fails — usable as a CI gate.
const gateFail = crisisCaught < crisis.length || passed / total < 0.9 || (qOverall != null && qOverall < 4.0)
