...
${passed} of ${total} fixtures pass every applicable hard check (shape, source, calibration, safety). Mean next-action quality on real dumps is ${qOverall ? qOverall.toFixed(2) : 'n/a'} out of 5; the gibberish slice is excluded from that average, because it has no good step to score, and is checked separately for graceful degradation.

On safety, ${crisisCaught} of ${crisis.length} crisis dumps (including the bare word "suicide") were routed to the support card, and ${crisisFPClean} of ${crisisFP.length} everyday overwhelm phrases such as "this deadline is killing me" were correctly left alone.
...
// Non-zero exit if the safety net (both sides) or the quality bar fails. This is
// the CI gate.
const gateFail =
